...
class FA_DETR(nn.Module):
    """ This is the FA-DETR module that performs object detection """
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels, aux_loss=True, with_box_refine=False): 
        ...
        num_pred = self.transformer.fa_decoder.num_layers
        if with_box_refine: 
            self.class_embed = _get_clones(self.class_embed, num_pred)
            self.bbox_embed = _get_clones(self.bbox_embed, num_pred)
            nn.init.constant_(self.bbox_embed[0].layers[-1].bias.data[2:], -2.0)
            # hack implementation for iterative bounding box refinement
            self.transformer.fa_decoder.bbox_embed = self.bbox_embed
        else:
            nn.init.constant_(self.bbox_embed.layers[-1].bias.data[2:], -2.0) 
            self.class_embed = nn.ModuleList([self.class_embed for _ in range(num_pred)])
            self.bbox_embed = nn.ModuleList([self.bbox_embed for _ in range(num_pred)])
            self.transformer.fa_decoder.bbox_embed = None
    ...
    def forward(self, samples, targets=None, supp_samples=None, supp_class_ids=None, supp_targets=None, class_prototypes=None):
        ...
        if self.training:
            ...
            class_prototypes, _ = self.compute_class_prototypes(supp_samples, supp_targets) 
        ...
        for i in range(num_episode): 
            if self.num_feature_levels == 1:
                if (support_batchsize * (i + 1)) <= num_support:
                    ...
                    if self.training:
                        episode_class_prototypes = class_prototypes[-support_batchsize:, :].unsqueeze(0).expand(batchsize, -1, -1) 
                    else:
            ...
            hs, init_reference, inter_references, query_feature = \
                self.transformer(srcs, masks, pos, query_embeds, episode_class_prototypes, 
            ...
            for lvl in range(hs.shape[0]): 
                ...
                if reference.shape[-1] == 4:
                    ...
                    tmp[..., :2] += reference
                outputs_coord = tmp.sigmoid() # （batchsize, num_queries, 4）


                # Calibration Module (CM) ===============================================
                boxes = box_ops.box_cxcywh_to_xyxy(outputs_coord) # （batchsize, num_queries, 4）
                ...
                if targets != None:
                    ...
                    for b in range(batchsize):
                        img_sizes.append([samples.tensors[b].shape[1], samples.tensors[b].shape[2]])
                    img_sizes = torch.tensor(img_sizes).to(device)
                ...
                rois = torchvision.ops.roi_align(
                    ...
                    aligned = True
                ).mean(3).mean(2) # batchsize * num_queries, C

                rois = rois.view(batchsize, self.num_queries, -1)
                roi_prototype_prob = torch.matmul(rois, episode_class_prototypes.transpose(1, 2)).sigmoid() # (batchsize, num_queries, episode_size)
                outputs_class_pred_prob = outputs_class.sigmoid() # (batchsize, num_queries, episode_size)
                outputs_class_pred_prob = outputs_class_pred_prob * roi_prototype_prob
                outputs_class = inverse_sigmoid(outputs_class_pred_prob)
                # ==========================================================================
            ...
            meta_support_class_ids.append(episode_class_ids) # (num_episode, episode_size)
        ...
        if self.args.class_prototypes_cls_loss:
            if self.num_feature_levels == 1:
                out['class_prototypes_cls_logits'] = self.class_prototypes_cls(class_prototypes) # (num_episode * episode_size, pred_class)
    ...
    def compute_class_prototypes(self, supp_samples, supp_targets): 
        ...
        if self.num_feature_levels == 1: 
            ...
            for i in range(num_episode): 
                class_prototypes, feat_before_RoIAlign_GAP = self.transformer.forward_supp_branch([srcs[0][i*self.args.episode_size: (i+1)*self.args.episode_size]], 
                ...
                class_prototypes_list.append(class_prototypes) 
            ...
            return class_prototypes, feat_before_RoIAlign_GAP # class_prototypes: (num_episode * episode_size, d_model)，feat_before_RoIAlign_GAP: (episode_size, hw, d_model)
...
class SetCriterion(nn.Module):
    """ This class computes the loss for FA-DETR.
    ...
    def loss_class_prototypes_cls(self, outputs, targets, indices, num_boxes):
        ...
        if len(logits) == len(targets):
            ...
            l_ce = 0
            for each_logit in logits:
                l_ce = l_ce + F.cross_entropy(each_logit, targets)
            l_ce = l_ce / len(logits)
            losses = {
                "loss_class_prototypes_cls": l_ce
            }
        return losses
