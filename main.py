...
from datasets.__init__ import build_dataset, get_coco_api_from_dataset
...
def get_args_parser():
    parser = argparse.ArgumentParser('FA-DETR', add_help=False)
    ...
    parser.add_argument('--class_prototypes_cls_loss', action='store_true', help="if set, enable class prototypes cls loss")
    ...
    parser.add_argument('--class_prototypes_cls_loss_coef', default=5.0, type=float)
    parser.add_argument('--focal_alpha', default=0.25, type=float)

    # dataset parameters
    parser.add_argument('--dataset_file', default='voc1')
...
def main(args):
    ...
    train_set = 'fewshot_finetune' if args.fewshot_finetune else 'base_train' 
    dataset_train = build_dataset(image_set=train_set, args=args) 
    dataset_val = build_dataset(image_set='val', args=args) 
    dataset_val_support = build_dataset(image_set='val_support_dataset', args=args) 

    if args.distributed:
        if args.cache_mode:
            sampler_train = samplers.NodeDistributedSampler(dataset_train)
            sampler_val = samplers.NodeDistributedSampler(dataset_val, shuffle=False)
            sampler_val_support = samplers.NodeDistributedSampler(dataset_val_support)
        else:
            sampler_train = samplers.DistributedSampler(dataset_train)
            sampler_val = samplers.DistributedSampler(dataset_val, shuffle=False)
            sampler_val_support = samplers.DistributedSampler(dataset_val_support)
    else:
        sampler_train = torch.utils.data.RandomSampler(dataset_train)
        sampler_val = torch.utils.data.SequentialSampler(dataset_val)
        sampler_val_support = torch.utils.data.RandomSampler(dataset_val_support)
    ...
    loader_val_support = DataLoader(dataset_val_support,
                                batch_size=1,
                                sampler=sampler_val_support,
                                drop_last=False,
                                num_workers=args.num_workers,
                                pin_memory=False)
            ...
            if args.class_prototypes_cls_loss:
                ...
                if args.num_feature_levels == 1:
                    for novel_class_id in novel_class_ids:
                        nn.init.normal_(model_without_ddp.class_prototypes_cls.L.weight[novel_class_id])
                elif args.num_feature_levels > 1:
                    for classifier in model_without_ddp.class_prototypes_cls:
                        for novel_class_id in novel_class_ids:
    ...
    if args.eval:
        test_stats, coco_evaluator = evaluate(
            args, model, criterion, postprocessors, loader_val, loader_val_support, base_ds, device, type='base'
        )
        test_stats, coco_evaluator = evaluate(
            args, model, criterion, postprocessors, loader_val, loader_val_support, base_ds, device, type='novel'
        )
        return 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_every_epoch == 0:
            if train_set == 'base_train': 
                evaltype = 'base'
            else: 
                evaltype = 'novel'

            test_stats, coco_evaluator = evaluate(
                args, model, criterion, postprocessors, loader_val, loader_val_support, base_ds, device, type=evaltype
            )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('FA-DETR', parents=[get_args_parser()])
