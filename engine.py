...
def evaluate(args, model, criterion, postprocessors, dataloader, support_data_loader, base_ds, device, type='all'):
    ...
    for i in range(number_of_supports):
        ...
        for i in range(num_episode):
            ...
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                class_prototypes, _ = model.module.compute_class_prototypes(support_images_, support_targets_)
            else:
                class_prototypes, _ = model.compute_class_prototypes(support_images_, support_targets_)
            # class_prototypes: (episode_size, d_model)


            category_codes_final.append(class_prototypes)
            support_class_ids_final.append(support_class_ids_)
        support_class_ids_final = torch.cat(support_class_ids_final, dim=0)
        category_codes_final = torch.cat(category_codes_final, dim=0)  # (num_episode x episode_size, d)
        all_category_codes_final.append(category_codes_final)

    if args.num_feature_levels == 1:
        all_category_codes_final = torch.stack(all_category_codes_final, dim=0)  # (number_of_supports(100), num_episode x episode_size, d)
        all_category_codes_final = torch.mean(all_category_codes_final, 0, keepdims=False) # (num_episode x episode_size, d)
    ...
    for samples, targets in metric_logger.log_every(dataloader, print_freq, header):
        ...
        all_category_codes_final_ = all_category_codes_final.expand(batchsize, -1, -1)

        outputs = model(samples, targets=targets, supp_class_ids=support_class_ids_final, class_prototypes=all_category_codes_final_)
        loss_dict = criterion(outputs)
        weight_dict = criterion.weight_dict
